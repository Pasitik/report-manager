...
export default function Accidents(props) {
  ...
  const updateContent = async (row) => {
    const accRef = await db.collection("Accidents").doc(row.id).get()
    if (accRef.exists) {
      const data = accRef.data()

      const solvedRef = await db.collection("SolvedAccidents").doc(row.id).get()
      if (!solvedRef.exists) {
        await db.collection("SolvedAccidents").doc(row.id).set(data).then(async (succ) => {
          await db.collection("Accidents").doc(row.id).delete()
        })
      }
    }
  }
